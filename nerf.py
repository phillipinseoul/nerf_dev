...
class HarmonicEmbedding(torch.nn.Module):
    def __init__(self, n_harmonic_functions=60, omega0=0.1):
       '''
       Takes input tensor `x`: [minibatch, ..., dim], and converts each feature in `x` 
       into a series of harmonic features `embedding` as follows:
    ...
    def forward(self, x):
        ...
        input:
            - x: [..., dim]
        output:
            - embedding: a `harmonic embedding of x`: [..., n_harmonic_functions * dim * 2]
        '''
